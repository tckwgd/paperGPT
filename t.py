...
def generate_paper_part(part_name, paper_requirements, bibtex_entries, previous_content="", extra_info="", word_limit=None, index=None, model=None):
    ...
    answer, tokens_used = generate_response_with_chat_model(prompt, model)
...
def generate_entire_paper(paper_requirements, bibtex_entries, model, word_limits):
    entire_paper = {}
    total_tokens = 0  # 初始化总的token消耗数
    
    intro_content, intro_tokens = generate_paper_part("introduction", paper_requirements, bibtex_entries, word_limit=word_limits["introduction"], model=model)
    entire_paper["introduction"] = intro_content
    total_tokens += intro_tokens
    
    entire_paper["creation"] = []
    for i in range(word_limits["creation_parts_count"]):
        creation_content, creation_tokens = generate_paper_part("creation", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["creation"][i], model=model)
        entire_paper["creation"].append(creation_content)
        total_tokens += creation_tokens
        
    related_content, related_tokens = generate_paper_part("related_work", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["related_work"], model=model)
    entire_paper["related_work"] = related_content
    total_tokens += related_tokens
    
    entire_paper["method"] = []
    for i in range(word_limits["method_parts_count"]):
        method_content, method_tokens = generate_paper_part("method", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["method"][i], index=i, model=model)
        entire_paper["method"].append(method_content)
        total_tokens += method_tokens
    
    conclusion_content, conclusion_tokens = generate_paper_part("conclusion", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["conclusion"], model=model)
    entire_paper["conclusion"] = conclusion_content
    total_tokens += conclusion_tokens
    
    discussion_content, discussion_tokens = generate_paper_part("discussion", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["discussion"], model=model)
    entire_paper["discussion"] = discussion_content
    total_tokens += discussion_tokens
    
    abstract_content, abstract_tokens = generate_paper_part("abstract", paper_requirements, bibtex_entries, entire_paper, word_limit=word_limits["abstract"], model=model)
    entire_paper["abstract"] = abstract_content
    ...
    ref_first_content, ref_first_tokens = generate_paper_part("reference", paper_requirements, first_half, entire_paper, model=model)
    entire_paper["reference_first"] = ref_first_content
    total_tokens += ref_first_tokens

    ref_second_content, ref_second_tokens = generate_paper_part("reference", paper_requirements, second_half, entire_paper, model=model)
    entire_paper["reference_second"] = ref_second_content
    total_tokens += ref_second_tokens
    
    return entire_paper, total_tokens
...
def main():
    ...
    if bib_docs:
        ...
        if "tokens_intro" not in st.session_state:
            st.session_state.tokens_intro = 0  # 初始化st.session_state中的tokens_intro为0

        word_limit_intro = st.number_input("介绍部分的字数要求:", min_value=50, value=500)
        
        if st.button("生成介绍"):
            generated_intro, tokens_for_intro = generate_paper_part("introduction", paper_requirements, bibtex_entries, word_limit=word_limit_intro, model=selected_model)
            st.session_state.previous_content["introduction"] = generated_intro
            st.session_state.tokens_intro = tokens_for_intro  # 更新st.session_state中的tokens_intro值
        ...
        st.write(f"Tokens used for introduction: {st.session_state.tokens_intro}")
        ...
        if "tokens_creation" not in st.session_state:
            st.session_state.tokens_creation = [0] * creation_parts_count

        for i in range(creation_parts_count):
            if st.button(f"生成创新点 - 部分 {i+1}"):
                generated_creation, tokens_for_this_creation = generate_paper_part("creation", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limits_creation[i], model=selected_model)
                ...
                st.session_state.tokens_creation[i] = tokens_for_this_creation  # 更新这一部分的token值
            ...
            st.write(f"Tokens used for creation part {i+1}: {st.session_state.tokens_creation[i]}")  # 使用st.session_state中保存的token值




        if "tokens_related_work" not in st.session_state:
            st.session_state.tokens_related_work = 0

        word_limit_related_work = st.number_input("相关工作部分的字数要求:", min_value=50, value=500)

        if st.button("生成相关工作"):
            generated_related_work, st.session_state.tokens_related_work = generate_paper_part("related_work", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limit_related_work, model=selected_model)
        ...
        st.write(f"Tokens used for related work: {st.session_state.tokens_related_work}")
        ...
        # 在session state中初始化tokens_method
        if "tokens_method" not in st.session_state:
            st.session_state.tokens_method = [0] * method_parts_count

        for i in range(method_parts_count):
            if st.button(f"生成方法 - 部分 {i+1}"):
                generated_method, st.session_state.tokens_method[i] = generate_paper_part("method", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limits_method[i], index=i, model=selected_model)
            ...
            st.write(f"Tokens used for method part {i+1}: {st.session_state.tokens_method[i]}")
        ...
        # 结论部分
        if "tokens_conclusion" not in st.session_state:
            st.session_state.tokens_conclusion = 0

        word_limit_conclusion = st.number_input("结论部分的字数要求:", min_value=50, value=500)
        if st.button("生成结论"):
            generated_conclusion, st.session_state.tokens_conclusion = generate_paper_part("conclusion", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limit_conclusion, model=selected_model)
        ...
        st.write(f"Tokens used for conclusion: {st.session_state.tokens_conclusion}")

        # 讨论部分
        if "tokens_discussion" not in st.session_state:
            st.session_state.tokens_discussion = 0

        word_limit_discussion = st.number_input("讨论部分的字数要求:", min_value=50, value=500)
        if st.button("生成讨论"):
            generated_discussion, st.session_state.tokens_discussion = generate_paper_part("discussion", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limit_discussion, model=selected_model)
        ...
        st.write(f"Tokens used for discussion: {st.session_state.tokens_discussion}")

        # 摘要部分
        if "tokens_abstract" not in st.session_state:
            st.session_state.tokens_abstract = 0

        word_limit_abstract = st.number_input("摘要部分的字数要求:", min_value=50, value=300)
        if st.button("生成摘要"):
            generated_abstract, st.session_state.tokens_abstract = generate_paper_part("abstract", paper_requirements, bibtex_entries, st.session_state.previous_content, word_limit=word_limit_abstract, model=selected_model)
        ...
        st.write(f"Tokens used for abstract: {st.session_state.tokens_abstract}")

        if 'tokens_first' not in st.session_state:
            st.session_state.tokens_first = 0

        if 'tokens_second' not in st.session_state:
            st.session_state.tokens_second = 0
        ...
        if st.button("生成参考文献"):
            ...
            # 为第一部分生成参考文献
            formatted_reference_first, st.session_state.tokens_first = generate_paper_part("reference", paper_requirements, first_half, st.session_state.previous_content, model=selected_model)
            st.write(formatted_reference_first)

            st.write("---")  # 可选的分隔线，使两部分更清晰

            # 为第二部分生成参考文献
            formatted_reference_second, st.session_state.tokens_second = generate_paper_part("reference", paper_requirements, second_half, st.session_state.previous_content, model=selected_model)
            st.write(formatted_reference_second)

        # Tokens used for references (放在if条件外面以确保每次都被显示)
        st.write(f"Tokens used for first half references: {st.session_state.tokens_first}")
        st.write(f"Tokens used for second half references: {st.session_state.tokens_second}")
        ...
        if st.button("一键生成论文"):
            ...
            paper_content, total_tokens_used = generate_entire_paper(paper_requirements, bibtex_entries, selected_model, word_limits)
            ...
            st.subheader("Token 消耗总数")
            st.write(total_tokens_used)
